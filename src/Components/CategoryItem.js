...
const CategoryItem = ({ title, description, viewLink ,categoryName ,icon, iconColor,pitchDeckLink, 
  documentationLink, openVideoModal,
  demoVideoLink,}) => {
  ...
  return (
    <div
    className="bg-white p-6 rounded-lg shadow-sm transition-transform duration-200 hover:scale-105 hover:shadow-md h-full flex flex-col"
  >
     {/* <div className="bg-white p-6 rounded-lg shadow-sm hover:shadow-md transition-shadow h-full flex flex-col"> */}
     <div className="mb-4">
     <FontAwesomeIcon 
          icon={icon} 
          className={`text-4xl mb-3 ${iconColor || 'text-blue-500 bg-blue-100 p-3 rounded-full'}`} /> 
          {icon && React.createElement(icon, { 
  className: `w-12 h-12 mb-3 ${iconColor || 'text-blue-500'} p-3 rounded-full bg-blue-100`
})}
      ...
      <p className="text-gray-600 mb-2">
        ...
        {description.length > descriptionLimit && !isExpanded && (
          <button onClick={toggleExpand} className="text-blue-500 ml-1 text-sm">
            Read more
          </button>
        )}
        {description.length > descriptionLimit && isExpanded && (
      ...
      {pitchDeckLink && (
          <a
            href={pitchDeckLink}
            target="_blank"
            rel="noopener noreferrer"  className="text-blue-600 hover:text-blue-800 font-light text-sm">
        ...
        {documentationLink && (
          ...
          {demoVideoLink && (
        <button
        onClick={() => openVideoModal(demoVideoLink)}
             className="text-blue-600 hover:text-blue-800 font-light text-sm">
